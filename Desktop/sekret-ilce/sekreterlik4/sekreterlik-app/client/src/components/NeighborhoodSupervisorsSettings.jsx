...
const NeighborhoodSupervisorsSettings = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [supervisorsData, districtsData, neighborhoodsData, membersData] = await Promise.all([
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingSupervisor) {
        await ApiService.updateNeighborhoodSupervisor(editingSupervisor.id, formData);
      } else {
        await ApiService.createNeighborhoodSupervisor(formData);
      }
  ...
  const handleEdit = (supervisor) => {
    ...
    setFormData({
      ...
      neighborhood_id: supervisor.neighborhood_id,
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-900">Mahalle Sorumluları</h2>
        <button
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {supervisors.map((supervisor) => (
                <tr key={supervisor.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {getNeighborhoodName(supervisor.neighborhood_id)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {getDistrictName(supervisor.neighborhood_id)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
        ...
        {supervisors.length === 0 && (
          <div className="text-center py-8 text-gray-500">
            Henüz mahalle sorumlusu eklenmemiş.
          </div>
        )}
      </div>
    </div>
...
export default NeighborhoodSupervisorsSettings;
