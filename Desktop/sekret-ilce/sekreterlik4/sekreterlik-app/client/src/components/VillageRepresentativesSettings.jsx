...
const VillageRepresentativesSettings = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [representativesData, districtsData, villagesData, membersData] = await Promise.all([
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingRepresentative) {
        await ApiService.updateVillageRepresentative(editingRepresentative.id, formData);
      } else {
        await ApiService.createVillageRepresentative(formData);
      }
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Bu temsilciyi silmek istediğinizden emin misiniz?')) {
      try {
        await ApiService.deleteVillageRepresentative(id);
        fetchData();
      } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-900">Köy Temsilcileri</h2>
        <button
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {representatives.map((representative) => (
                <tr key={representative.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {getVillageName(representative.village_id)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {getDistrictName(representative.village_id)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
        ...
        {representatives.length === 0 && (
          <div className="text-center py-8 text-gray-500">
            Henüz köy temsilcisi eklenmemiş.
          </div>
        )}
      </div>
    </div>
...
export default VillageRepresentativesSettings;
