...
const CreateEventForm = ({ onClose, onEventCreated, members }) => {
        ...
        setAttendance(prev => {
          ...
          responsible.forEach(member => {
            ...
            if (!(stringId in newAttendance)) {
              newAttendance[stringId] = { status: 'attended', excuseReason: '' };
            }
          });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Get selected category name - use category name as event name
      ...
      const eventData = {
        ...
        attendees: sortedMembers.map(member => {
          ...
          // Backward compat: old format was boolean, new format is object
          let attended = false;
  ...
  const getAttendanceStatus = (memberId) => {
    ...
    // Backward compat: old format was boolean
    ...
    <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4 md:space-y-6">
                      ...
                      <div className="flex items-center space-x-3">
                        <label className="flex items-center">
                          <input
                            ...
                            className="h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300"
                          />
                          <span className="ml-1 text-xs sm:text-sm text-gray-700">Katıldı</span>
                        </label>
                        <label className="flex items-center">
                          <input
                            type="radio"
                            ...
                            className="h-4 w-4 text-red-600 focus:ring-red-500 border-gray-300"
                          />
                          <span className="ml-1 text-xs sm:text-sm text-gray-700">Katılmadı</span>
                        </label>
                        <label className="flex items-center">
                          <input
                            type="radio"
                            ...
                            className="h-4 w-4 text-amber-600 focus:ring-amber-500 border-gray-300"
                          />
                          <span className="ml-1 text-xs sm:text-sm text-gray-700">Mazeretli</span>
                        </label>
                      </div>
                      {getAttendanceStatus(member.id) === 'excused' && (
                        <input
                          ...
                          onChange={(e) => handleExcuseReasonChange(member.id, e.target.value)}
                          placeholder="Mazeret sebebi"
                          className="mt-1 w-full px-2 py-1 border border-gray-300 rounded-md text-xs sm:text-sm"
                        />
                      )}
                    </div>
              </div>
      ...
      <div className="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3 pt-4 sm:pt-6 pb-4 sm:pb-0 border-t border-gray-200 dark:border-gray-700 sticky bottom-0 bg-white dark:bg-gray-800 -mx-4 sm:-mx-6 px-4 sm:px-6 mt-4 sm:mt-0">
        <button
          type="button"
          onClick={onClose}
          className="w-full sm:w-auto px-4 py-2.5 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-200 rounded-lg transition-colors duration-200"
        >
          İptal
        </button>
        <button
