  ...
  useEffect(() => {
    if (member) {
      setFormData({
        ...
        inspectorDistrict: member.inspectorDistrict || '',
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {formData.inspectorTitle && (
            <div>
              <label htmlFor="member-inspectorDistrict" className="block text-sm font-medium text-gray-700 mb-1">
                Müfettiş Olduğu İlçe
              </label>
              <input
                id="member-inspectorDistrict"
                name="inspectorDistrict"
                type="text"
                value={formData.inspectorDistrict}
                onChange={handleChange}
                placeholder="Örn: Merkez, Karakoçan..."
                className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all shadow-sm"
              />
            </div>
          )}
