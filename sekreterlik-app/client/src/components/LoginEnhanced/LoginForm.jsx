    ...
    <form className="space-y-5" onSubmit={handleSubmit} autoComplete="off">
      <AnimatePresence>
        {error && (
          <motion.div
            ...
            className="bg-red-50/80 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 rounded-xl p-4 shadow-sm"
          >
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input
            id="username" name="username" type="text" autoComplete="off" required autoFocus
            value={username} onChange={(e) => setUsername(e.target.value)}
            className="block w-full pl-11 pr-4 py-3 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all text-sm font-medium dark:text-white"
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          {/* "Şifremi Unuttum" linki kaldırıldı — sistem şu an SMS OTP kullanmıyor.
              Üye şifresini unutursa il başkanlığını arayıp manuel reset isteyecek. */}
        </div>
        <div className="relative">
          <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
            <svg className="h-4 w-4 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
        ...
        {capsLock && <p className="text-amber-600 text-[10px] font-bold mt-1 uppercase tracking-wider">Caps Lock Açık</p>}
      </div>
      ...
      <div className="flex items-center">
        <input
          ...
          className="h-4 w-4 text-primary-600 focus:ring-primary-500/20 border-gray-300 dark:border-gray-700 rounded-md bg-white dark:bg-gray-800"
        />
      ...
      <motion.button
        ...
        type="submit"
        disabled={isLoading}
        className="w-full flex justify-center py-4 px-4 border border-transparent rounded-xl text-sm font-bold text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 shadow-lg shadow-primary-500/25 transition-all duration-300 disabled:opacity-70 disabled:cursor-not-allowed"
      >
        {isLoading ? (
