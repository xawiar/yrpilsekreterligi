...
const NeighborhoodRepresentativesSettings = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [representativesData, districtsData, neighborhoodsData, membersData] = await Promise.all([
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingRepresentative) {
        await ApiService.updateNeighborhoodRepresentative(editingRepresentative.id, formData);
      } else {
        await ApiService.createNeighborhoodRepresentative(formData);
      }
  ...
  const handleEdit = (representative) => {
    ...
    setFormData({
      ...
      neighborhood_id: representative.neighborhood_id,
  ...
  const getNeighborhoodName = (neighborhoodId) => {
    ...
    return neighborhood ? neighborhood.name : 'Bilinmiyor';
  };

  const getDistrictName = (neighborhoodId) => {
    const neighborhood = neighborhoods.find(n => n.id === neighborhoodId);
    if (!neighborhood) return 'Bilinmiyor';
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-900">Mahalle Temsilcileri</h2>
        <button
      ...
      {showForm && (
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h3 className="text-lg font-semibold mb-4">
            {editingRepresentative ? 'Temsilci Düzenle' : 'Yeni Temsilci Ekle'}
          </h3>
                ...
                >
                  ...
                  {neighborhoods.map(neighborhood => (
                    <option key={neighborhood.id} value={neighborhood.id}>
                      {neighborhood.name} - {districts.find(d => d.id === neighborhood.district_id)?.name}
                    </option>
                  ))}
                </select>
              </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {representatives.map((representative) => (
                <tr key={representative.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {getNeighborhoodName(representative.neighborhood_id)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {getDistrictName(representative.neighborhood_id)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {getMemberName(representative.member_id)}
                  </td>
        ...
        {representatives.length === 0 && (
          <div className="text-center py-8 text-gray-500">
            Henüz mahalle temsilcisi eklenmemiş.
          </div>
        )}
      </div>
    </div>
...
export default NeighborhoodRepresentativesSettings;
