  ...
  useEffect(() => {
    const loadPerformanceScore = async () => {
      ...
      try {
        ...
        // Determine if member is new (no activity at all)
        const isNewMember = score.totalScore === 0 &&
          score.details.meetingAttendance === 0 &&
          score.details.eventAttendance === 0 &&
          score.details.memberRegistrations === 0;

        setPerformanceScore(score);
        setMaxScore(max);
        setLevelInfo({
          level: isNewMember ? 'Yeni Üye' : level,
          levelColor: isNewMember ? '#6366F1' : levelColor,
          stars: averageStars,
          performanceStars,
      ...
      {(performanceScore || levelInfo) && (
        <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl p-4 sm:p-5 border border-indigo-200 mb-4 sm:mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            ...
            <div className="bg-white p-3 sm:p-4 rounded-lg border border-gray-200 shadow-sm">
              <div className="text-xs sm:text-sm text-gray-500 mb-1">Performans</div>
              <div className="flex items-center gap-1">
                {loadingScore ? (
                  <span className="text-lg sm:text-xl">...</span>
                ) : levelInfo?.isNewMember ? (
                  <span className="text-sm text-indigo-600 dark:text-indigo-400 font-medium">Yeni Uye</span>
                ) : (
                  Array.from({ length: 5 }).map((_, i) => (
                    <svg
                      key={i}
                      className={`w-5 h-5 sm:w-6 sm:h-6 ${i < (levelInfo?.performanceStars || 0) ? 'text-yellow-400 fill-current' : 'text-gray-300'}`}
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
              ...
              {levelInfo?.manualStars !== null && levelInfo?.manualStars !== undefined && (
                <div className="mt-2">
                  <div className="text-xs text-gray-500 mb-1">Yonetici Degerlendirmesi</div>
                  <div className="flex items-center gap-1">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <svg
                        key={i}
                        className={`w-4 h-4 sm:w-5 sm:h-5 ${i < levelInfo.manualStars ? 'text-purple-400 fill-current' : 'text-gray-300'}`}
                        fill="currentColor"
                        viewBox="0 0 20 20"
                      >
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
