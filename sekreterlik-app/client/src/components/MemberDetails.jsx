              ...
              <div className="flex items-center gap-1">
                {loadingScore ? (
                  ...
                  Array.from({ length: 5 }).map((_, i) => {
                    const starsToShow = levelInfo?.averageStars !== undefined ? levelInfo.averageStars : (levelInfo?.stars || 0);
                    return (
                      <svg
                        key={i}
                        className={`w-5 h-5 sm:w-6 sm:h-6 ${i < starsToShow ? 'text-yellow-400 fill-current' : 'text-gray-300'}`}
                        fill="currentColor"
                        viewBox="0 0 20 20"
                      >
