...
const TownMembersPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(`Hata: ${error.message || 'Üye kaydedilirken bir hata oluştu'}`);
    }
  };
  ...
  const handleDelete = async (memberId) => {
    const confirmed = await confirm({ title: 'Üye Sil', message: 'Bu üyeyi silmek istediğinizden emin misiniz?' });
    if (!confirmed) return;
    try {
      ...
      toast.error(`Hata: ${error.message || 'Üye silinirken bir hata oluştu'}`);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{town.name} Yönetim Kurulu</h1>
          <p className="text-gray-600 dark:text-gray-400">
            {district?.name} ilçesine bağlı belde yönetim kurulu üyeleri
          </p>
        </div>
      ...
      {filteredMembers.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {searchTerm ? 'Arama kriterlerinize uygun üye bulunamadı.' : 'Bu beldeye ait henüz üye eklenmemiş.'}
          </p>
        </div>
      ) : (
...
export default TownMembersPage;
