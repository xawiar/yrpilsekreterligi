...
const GenclikKollarıPage = () => {
  ...
  const fetchPresidents = async () => {
    try {
      const data = await ApiService.getYouthBranchPresidents();
      const presidentsMap = {};
      data.forEach(p => {
  ...
  const fetchManagement = async (presidentMemberId) => {
    try {
      const data = await ApiService.getYouthBranchManagement(presidentMemberId);
      setManagement(data || []);
    } catch (error) {
  ...
  const handleSetPresident = async (memberId) => {
    ...
    try {
      ...
      toast.success('Gençlik kolları başkanı başarıyla atandı');
    } catch (error) {
  ...
  const handleRemovePresident = async () => {
    ...
    try {
      ...
      toast.success('Gençlik kolları başkanlığı kaldırıldı');
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-gray-100">Gençlik Kolları</h1>
        <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
          Bölge bazında gençlik kolları başkanlığı yönetimi
        </p>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-blue-900 dark:text-blue-100">
                Mevcut Gençlik Kolları Başkanı
              </h3>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 p-6">
          <div className="mb-4">
            <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-2">
              Gençlik Kolları Yönetimi
            </h2>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {selectedRegion} bölgesi gençlik kolları başkanının oluşturduğu yönetim listesi
            </p>
          </div>
              ...
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredMembers.map((member) => {
                    ...
                    return (
                      <tr key={member.id} className={isPresident ? 'bg-blue-50 dark:bg-blue-900/20' : ''}>
...
export default GenclikKollarıPage;
