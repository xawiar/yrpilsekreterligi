...
const { decryptField, encryptField } = require('../utils/crypto');
  ...
  static async create(req, res) {
    try {
      ...
      if (is_chief_observer) {
        try {
          // TC ile üye bul - önce şifreli, sonra düz metin ile ara (O(1) sorgu)
          let encryptedTc;
          try { encryptedTc = encryptField(tc); } catch (_) { encryptedTc = null; }
          let member = encryptedTc
            ? await db.get('SELECT * FROM members WHERE tc = ? AND (archived = 0 OR archived IS NULL)', [encryptedTc])
            : null;
          if (!member) {
            member = await db.get('SELECT * FROM members WHERE tc = ? AND (archived = 0 OR archived IS NULL)', [tc]);
          }
          ...
          if (member && member.id) {
            ...
            if (!existingUser) {
              ...
              console.log(`Basmusahit kullanicisi olusturuldu: Member ID: ${member.id}, Username: ${username}`);
            } else if (existingUser.username !== username) {
              await db.run('UPDATE member_users SET username = ?, password = ? WHERE id = ?', [username, password, existingUser.id]);
              console.log(`Basmusahit kullanici adi guncellendi: ${existingUser.username} -> ${username}`);
            }
          } else {
            console.warn(`Basmusahit icin uye bulunamadi (TC: ${tc}), kullanici olusturulmadi`);
          }
        } catch (userError) {
          console.error('Basmusahit kullanicisi olusturulurken hata:', userError);
        }
      }
    ...
    try {
      ...
      if (is_chief_observer) {
        try {
          ...
          console.error('Basmusahit kullanicisi guncellenirken hata:', userError);
        }
      }
  ...
  static async delete(req, res) {
    try {
      ...
      try {
        ...
        const usersByUsername = await db.all('SELECT * FROM member_users WHERE username = ?', [username]);
